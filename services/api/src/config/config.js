...
class Config {
  ...
  loadEnvironmentConfig() {
    ...
    // JWT configuration
    // SECURITY: No fallback for JWT_SECRET - must be set in environment
    if (!process.env.JWT_SECRET) {
      throw new Error(
        'SECURITY ERROR: JWT_SECRET environment variable is required. ' +
        'Generate a secure secret using: node -e "logger.info(require(\'crypto\').randomBytes(64).toString(\'hex\'))"'
      );
    }
    this.set("jwt", {
