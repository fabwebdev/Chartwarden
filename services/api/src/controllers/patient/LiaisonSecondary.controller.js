...
export const store = async (request, reply) => {
  try {
    logger.info("🔍 LiaisonSecondary store called - User:", request.user?.id)
    console.log(
    ...
    logger.info("✅ LiaisonSecondary store - Created liaison:", result?.id)
    ...
    logger.error("❌ Error in LiaisonSecondary store:", error)
