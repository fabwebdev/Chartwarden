...
class LivingArrangementsController {
    ...
    async index(request, reply) {
        try {
            const livingArrangements = await db.select({
                id: living_arrangements.id,
                patient_id: living_arrangements.patient_id,
                primary_caregiver: living_arrangements.primary_caregiver,
                primary_location_of_patient: living_arrangements.primary_location_of_patient,
                caregiver_availability: living_arrangements.caregiver_availability,
                patient_able: living_arrangements.patient_able,
                need_hospice_service: living_arrangements.need_hospice_service,
                createdAt: living_arrangements.createdAt,
                updatedAt: living_arrangements.updatedAt,
            }).from(living_arrangements);
            reply.code(200);
            return livingArrangements;
        } catch (error) {
            logger.error("Error fetching living arrangements:", error)
            reply.code(500);
            return {
    ...
    async livingArrangementsList(request, reply) {
        try {
            ...
            logger.error("Error fetching living arrangements list:", error)
            reply.code(500);
            return {
                message: "Internal server error",
                error: error.message,
    ...
    async store(request, reply) {
        try {
            ...
            const existingArrangements = await db.select({ id: living_arrangements.id }).from(living_arrangements).where(eq(living_arrangements.patient_id, patient_id)).limit(1);
            ...
            logger.error("Error storing living arrangements:", error)
            reply.code(500);
            return {
                message: "Internal server error",
                error: error.message,
    ...
    async show(request, reply) {
        try {
            ...
            const livingArrangementsRecords = await db.select({
                id: living_arrangements.id,
                patient_id: living_arrangements.patient_id,
                primary_caregiver: living_arrangements.primary_caregiver,
                primary_location_of_patient: living_arrangements.primary_location_of_patient,
                ...
                createdAt: living_arrangements.createdAt,
                updatedAt: living_arrangements.updatedAt,
            }).from(living_arrangements).where(eq(living_arrangements.patient_id, id)).limit(1);
