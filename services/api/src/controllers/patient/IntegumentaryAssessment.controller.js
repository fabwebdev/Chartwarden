...
class IntegumentaryAssessmentController {
    ...
    async show(request, reply) {
        try {
            ...
            logger.info("🔍 IntegumentaryAssessmentController.show called with patient_id:", id)
            ...
            logger.info("📊 Found assessments:", integumentaryAssessments.length)
            const integumentaryAssessment = integumentaryAssessments[0];

            if (!integumentaryAssessment) {
                logger.info("ℹ️ No integumentary assessment found for patient_id:", id, "- returning empty data")
            ...
            logger.info("✅ Found integumentary assessment:", integumentaryAssessment.id)
            reply.code(200);
            return integumentaryAssessment;
        } catch (error) {
            logger.error("❌ Error fetching integumentary assessment:", error)
            reply.code(500);
            return {
                message: "Internal server error",
                error: error.message,
