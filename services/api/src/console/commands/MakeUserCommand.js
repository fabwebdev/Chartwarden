    ...
    try {
      ...
      if (role) {
        ...
        if (roleResult.length > 0) {
          ...
          logger.info(`Role '${role}' assigned successfully!`)
        } else {
          logger.info(`Role '${role}' not found. Available roles: admin, doctor, nurse, patient, staff`)
          ...
          if (patientRole.length > 0) {
            ...
            logger.info("Assigned default 'patient' role.")
          }
        }
      }
    } catch (error) {
