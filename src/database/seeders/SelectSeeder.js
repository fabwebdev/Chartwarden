...
class SelectSeeder {
  ...
  async run() {
    try {
      ...
      // Create test patient
      await db.execute(sql`
        INSERT INTO ${patients} (first_name, mi, last_name, preferred_name, date_of_birth, suffix, oxygen_dependent, ssn, patient_consents, genders, dme_provider, primary_diagnosis_id, patient_pharmacy_id, dnr_id, race_ethnicity_id, emergency_preparedness_level_id, liaison_primary_id, liaison_secondary_id, hipaa_received)
        VALUES ('Ahlin', 'eeee', 'Byll', 'pppp', '2024-04-03', 'dddd', 1, 666, 0, 'male', 'wheelchair', 1, 1, 1, 1, 1, 1, 1, 0)
        ON CONFLICT (first_name, last_name, date_of_birth) DO NOTHING
      `);

      logger.info('SelectSeeder completed successfully.')
    } catch (error) {
      logger.error('Error in SelectSeeder:', error)
    }
  }
}
