...
// TYPES
import { Accordion, AccordionDetails, AccordionSummary, Autocomplete, Box, Button, CardContent, Checkbox, CircularProgress, Divider, FormControlLabel, FormGroup, Grid, IconButton, InputLabel, ListItemIcon, ListItemSecondaryAction, MenuItem, Modal, Select, Stack, TextField, Typography, AccordionSummaryProps } from '@mui/material';
...
const EditPatientPage = ({ patientsId }: { patientsId: number }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    http
      .put(`/patient/${patientsId}`, payload)
      .then((response: any) => {
        Swal.fire({
          icon: 'success',
          title: 'Patient Added',
  ...
  const patientById = async () => {
    http
      ...
      .then((response: any) => {
        ...
        if (response.data.primary_diagnosis_code && response.data.primary_diagnosis_description) {
          setSelectedPrimaryDiagnosis({
            code: response.data.primary_diagnosis_code,
            description: response.data.primary_diagnosis_description,
            id: response.data.primary_diagnosis_id
          });
        } else if (response.data.primary_diagnosis_id) {
          // If we only have ID, try to find it in the list after it loads
        ...
        if (response.data.races) {
          // If races is an array, use it directly
          setSelectedRaces(Array.isArray(response.data.races) ? response.data.races : []);
        } else if (response.data.race) {
          // If race is a comma-separated string, split it
          const raceArray = typeof response.data.race === 'string' 
            ? response.data.race.split(',').map((r: string) => r.trim()).filter(Boolean)
            : [];
          setSelectedRaces(raceArray);
        }
        
        if (response.data.ethnicities) {
          // If ethnicities is an array, use it directly
          setSelectedEthnicities(Array.isArray(response.data.ethnicities) ? response.data.ethnicities : []);
        } else if (response.data.ethnicity) {
          // If ethnicity is a comma-separated string, split it
          const ethnicityArray = typeof response.data.ethnicity === 'string'
            ? response.data.ethnicity.split(',').map((e: string) => e.trim()).filter(Boolean)
            : [];
          setSelectedEthnicities(ethnicityArray);
        }
        
        setSelected({
          gender: response.data.gender || response.data.genders || '',
          patient_pharmacy_id: response.data.patient_pharmacy_id || '',
          dme_provider: response.data.dme_provider || response.data.dme_provider_id || '',
          primary_diagnosis_id: (response.data.primary_diagnosis_id && typeof response.data.primary_diagnosis_id === 'number') ? response.data.primary_diagnosis_id.toString() : (response.data.primary_diagnosis_code || ''),
          hipaa_received: response.data.hipaa_received || '',
          dnr_id: response.data.dnr_id || '',
          race_ethnicity_id: response.data.race_ethnicity_id || '',
          liaison_primary_id: response.data.liaison_primary_id || '',
          emergency_preparedness_level_id: response.data.emergency_preparedness_level_id || '',
          oxygen_dependent: response.data.oxygen_dependent || '',
          patient_consents: response.data.patient_consents || '',
          liaison_secondary_id: response.data.liaison_secondary_id || '',
          veterans_status: response.data.veterans_status !== undefined ? (response.data.veterans_status === 1 || response.data.veterans_status === true || response.data.veterans_status === "1" || response.data.veterans_status === "true" ? true : false) : false,
          // Assurez-vous de mapper toutes les autres propriétés nécessaires
        });
      })
      .catch((error: any) => {});
  };
  ...
  // Match existing primary diagnosis when list loads
  useEffect(() => {
    if (primaryDiagnosisList.length > 0 && patientData.primary_diagnosis_id && !selectedPrimaryDiagnosis) {
      // Try to find the diagnosis in the list
      const foundDiagnosis = primaryDiagnosisList.find(
        (diag) => diag.id === patientData.primary_diagnosis_id || diag.code === patientData.primary_diagnosis_id
      );
      if (foundDiagnosis) {
        setSelectedPrimaryDiagnosis(foundDiagnosis);
      } else if (patientData.primary_diagnosis_code && patientData.primary_diagnosis_description) {
        // If not found but we have code and description, create a match
        setSelectedPrimaryDiagnosis({
          code: patientData.primary_diagnosis_code,
          description: patientData.primary_diagnosis_description,
          id: patientData.primary_diagnosis_id
        });
      }
    }
  }, [primaryDiagnosisList, patientData.primary_diagnosis_id]);
            ...
            <Grid item xs={12} sm={6} md={2}>
              <Stack spacing={1.25}>
                ...
                <Select
                  fullWidth
                  id="received"
                  value={selected.patient_consents ? "1" : "0"}
                  onChange={(event: any) => {
                    const selectedId = event.target.value === "1";
            ...
            <Grid item xs={12} sm={6} md={2}>
              <Stack spacing={1.25}>
                ...
                <Select
                  fullWidth
                  id="received"
                  value={selected.hipaa_received ? "1" : "0"}
                  onChange={(event: any) => {
                    const selectedId = event.target.value === "1";
                    setSelected((prevSelected) => ({
                      ...prevSelected,
            ...
            <Grid item xs={12} sm={6} md={2}>
              <Stack spacing={1.25}>
                ...
                <Select
                  fullWidth
                  id="oxygen"
                  value={selected.oxygen_dependent ? "1" : "0"}
                  onChange={(event: any) => {
                    const selectedId = event.target.value === "1";
                    setSelected((prevSelected) => ({
                      ...prevSelected,
            ...
            <Grid item xs={12} sm={6} md={2}>
              <Stack spacing={1.25}>
                ...
                <Select
                  fullWidth
                  id="veterans_status"
                  value={selected.veterans_status !== undefined ? (selected.veterans_status === true ? "1" : "0") : (patientData.veterans_status !== undefined ? (patientData.veterans_status === 1 || patientData.veterans_status === true || patientData.veterans_status === "1" || patientData.veterans_status === "true" ? "1" : "0") : "0")}
                  onChange={(event: any) => {
                    const selectedId = event.target.value === "1";
                    setSelected((prevSelected) => ({
                      ...prevSelected,
                    ...
                    setPatientData((prevPatientData: any) => ({
                      ...prevPatientData,
                      veterans_status: selectedId ? "1" : "0"
                    }));
                  }}
                >
                  <MenuItem value="1">Yes</MenuItem>
            ...
            <Grid item xs={12} sm={6} md={4}>
              <Stack spacing={1.25}>
                ...
                <Select
                  fullWidth
                  id="Provider"
                  value={selected.dme_provider || patientData.dme_provider || ''}
                  onChange={(event: any) => {
