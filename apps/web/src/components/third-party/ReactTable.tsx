...
export const CSVExport = ({ data, filename, headers }: CSVExportProps) => {
  // Validate data format before passing to CSVLink
  // CSVLink expects: string, array of arrays, or array of objects
  const isValidData = 
    typeof data === 'string' ||
    (Array.isArray(data) && (
      data.length === 0 ||
      (Array.isArray(data[0]) || (typeof data[0] === 'object' && data[0] !== null && !Array.isArray(data[0])))
    ));

  if (!isValidData) {
    console.warn('CSVExport: Invalid data format', data);
    return null;
  }
